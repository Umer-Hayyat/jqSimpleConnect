...
jqSimpleConnect._positionConnection = function (connection) {
    ...
    var line1 = $(`#${connection.id}_1`);
    var line2 = $(`#${connection.id}_2`);
    var line3 = $(`#${connection.id}_3`);
 ...
 *
 *@param {object} jqElement A $ object of the HTML element used for represent the line.
 ...
 *
 * @param {object} elementA A CSS selector or a $'s object for select the first element.
 * @param {object} elementB A CSS selector or a $'s object for select the second element.
 ...
 */
jqSimpleConnect.connect = function (elementA, elementB, options, parentContainer, connectionEventCallbacks) {
    // Verify if the element's selector are ok.
    if (elementA == null || $(elementA).length == 0 ||
        elementB == null || $(elementB).length == 0) {
        return null;
    }
    ...
    $(parentContainer || 'body').prepend(div.replace('divUniqueIdentifier', `${connection.id}_1`));
    $(parentContainer || 'body').prepend(div.replace('divUniqueIdentifier', `${connection.id}_2`));
    $(parentContainer || 'body').prepend(div.replace('divUniqueIdentifier', `${connection.id}_3`));

    //attach events
    jqSimpleConnect._attachEvents(connection, connectionEventCallbacks);
 ...
 */
jqSimpleConnect._attachEvents = function (connection, connectionEventCallbacks) {
    // Get the line's elements.
    var line1 = $(`#${connection.id}_1`).click(connectionEventCallbacks.click);
    var line2 = $(`#${connection.id}_2`).click(connectionEventCallbacks.click);
    var line3 = $(`#${connection.id}_3`).click(connectionEventCallbacks.click);
}
...
jqSimpleConnect.removeConnection = function (connectionId) {
    if (jqSimpleConnect._connections[connectionId] != null) {
        // Remove HTML element.
        $(`.jqSimpleConnect.${connectionId}`).remove();
